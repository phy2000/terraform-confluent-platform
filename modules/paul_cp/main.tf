...
resource "aws_instance" "app" {
    ...
    tags = {
        Name = "${data.template_file.cluster_name_prefix.rendered}-app${count.index+1}"
        Owner = var.first_name
        owner = var.first_name
        SA = var.first_name
        sa = var.first_name
...
resource "aws_route53_record" "app" {
    count   = var.app_servers
    zone_id = data.aws_route53_zone.public.zone_id
    name    = "app${count.index+1}.${data.template_file.cluster_dns_postfix.rendered}"
    type    = "CNAME"
    ttl     = "300"
    records = [element(aws_instance.app.*.public_dns, count.index)]
}
